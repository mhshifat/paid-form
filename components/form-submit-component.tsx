...
export default function FormSubmitComponent({ content, formUrl }: { content: FormElementInstance[], formUrl: string }) {
  ...
  const submitForm = async () => {
    ...
    try {
      ...
      await saveForm(formUrl, jsonContent);
      setSubmitted(true);
    } catch (err) {
      toast({
        title: "Error",
    ...
    <div className="flex justify-center w-full h-full items-center p-8">
      <div key={renderKey} className="max-w-[620px] flex flex-col gap-4 flex-grow bg-background w-full p-8 overflow-y-auto border shadow-xl shadow-blue-700 rounded">
        {content.map(el => {
          ...
          return (
            <FormElement key={el.id} defaultValue={formValues.current[el.id]} elementInstance={el} submitValue={submitValue} isInvalid={formErrors.current[el.id]} />
          )
        })}
